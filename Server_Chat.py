...
def atender_cliente(client_socket, address):
    ...
    try:
        while username is None:
            ...
            if opcion == "registrar":
                if db_backup.existe_usuario(user_input):
                    client_socket.send("[ERROR] Ese usuario ya existe. Probá de nuevo.\n".encode("utf-8"))
                    continue
                db_backup.registrar_usuario(user_input, pass_input)
                username = user_input
                client_socket.send(f"[OK] Usuario {username} registrado correctamente.\n".encode("utf-8"))

            elif opcion == "login":
                if db_backup.verificar_usuario(user_input, pass_input):
                    username = user_input
    ...
    db_backup.registrar_conexion(username, "conexion")
    ...
    while True:
        try:
                    ...
                    if respuesta_api.status_code == 200:
                        ...
                        if isinstance(datos_json, list) and len(datos_json) > 0:
                            ...
                            if isinstance(primer_gatito, dict) and "url" in primer_gatito:
                                ...
                                db_backup.guardar_api_call(username, "/gato", url_imagen_gato)
            ...
            db_backup.guardar_mensaje(username, data)
    ...
    db_backup.registrar_conexion(username, "desconexion")
